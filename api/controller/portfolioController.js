    ...
    //Autorite ja siltide lisamine
     ...
     // Size vajab muutmist
      if(req.files.document){
        const document={     
            file_loc_url: 'http://localhost:3000/public/uploads/files/'+req.files.document[0].filename,
         ...
         await portfolioService.file(document);
        }
        ...
        const targetPortfolioThumbnail = 'C:/Users/tooma/Desktop/riiul/public/uploads/portfolioThumbnails/'
        const targetThumbnail = 'C:/Users/tooma/Desktop/riiul/public/uploads/thumbnails/'+name
       ...
       const thumbnailsize = sharp(req.files.files[0].path).rotate().resize( 200,200).toFile(targetThumbnail).then((data)=>{ data.size}).catch((err)=>console.log(err))
        ...
        const thumbnail= {
            file_loc_url: 'http://localhost:3000/public/uploads/thumbnails/'+name,
            name: name,
            text:phototext1,
            type:"thumbnail",
            portfolio_ID: portfolio_id,
        ...
        for(i=0; i<req.files.files.length; i++){
            const photos = {
                file_loc_url: 'http://localhost:3000/public/uploads/photos/'+req.files.files[i].filename,
            ...
            const portfolioThumbnail= {
                file_loc_url: 'http://localhost:3000/public/uploads/portfolioThumbnails/'+portfolioThumbnailName,
